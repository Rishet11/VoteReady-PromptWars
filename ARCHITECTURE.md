...
sequenceDiagram
    ...
    Frontend->>NextApi: POST { stateCode: "DL" }
    NextApi->>NextApi: Sanitize Input & Lookup Election Data
    NextApi->>NextApi: Construct System & User Prompt
    NextApi->>Gemini: sendMessage(Prompt)
    Note over Gemini: 3-second Timeout
    Gemini-->>NextApi: Response Text
    NextApi-->>Frontend: { guidance: "..." }
...
3. **Graceful Degradation:** If the Gemini API times out or fails, a predefined hardcoded "Fallback Guidance" is returned so the user experience is never blocked.
